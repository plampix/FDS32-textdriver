...
void fdsScreen::setPins() {
  setPins(13, 11, 8, 2000);
}

void fdsScreen::setPins(int p_clockPin, int p_dataPin, int p_globalClockPin, int p_delay_us) {
  clockPin  = p_clockPin;
  dataPin = p_dataPin;
  globalClockPin = p_globalClockPin;
  delay_us = p_delay_us;
  pinMode (clockPin, OUTPUT);
  pinMode (dataPin, OUTPUT);
  pinMode(globalClockPin, OUTPUT);
  digitalWrite(globalClockPin, HIGH);
...
{                           
    ...
    setRow(row);
    delayMicroseconds(delay_us);         // pause, because otherwise it will update too quickly
  }
}
...
void fdsScreen::dump()
{
  for (int row=0; row<7; row++)
  {
    for (int i=0;i<=34; i++)
    {
      for (int p=1, b=7;b>=0;b--,p<<=1)
      {
        Serial.print(output[row][i] & p ? "@" : ".");
      }
    }
    Serial.println();
  }
